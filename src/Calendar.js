...
export const CustomCalendar = () => {
    ...
    const fetchData = async (startDate, endDate) => {
        ...
        if (eventsDummy) {
            ...
            const filterDataArr = {
                companyName: [...new Set(eventsDummy.map(event => event['v_corporate_actions.long_name_en']))],
    ...
    const handleSelectDate = value => {
        ...
        if (filteredEvents.length > 0) {
            ...
            filteredEvents.map(event => {
                if (event['v_corporate_actions.entry_type'] === 'Action' && event['v_corporate_actions.action_type'] === 'Dividend') {
                    actionTypesData['Dividend'].push(event);
                } else if (event['v_corporate_actions.entry_type'] === 'Event' && event['v_corporate_actions.action_type'] === 'General Assembly Meeting') {
                    actionTypesData['General Assembly Meeting'].push(event);
                } else if (event['v_corporate_actions.entry_type'] === 'Event' && event['v_corporate_actions.action_type'] === 'Board of Directors Session') {
    ...
    const onFiltersUpdate = (filtersList) => {
        ...
        const filteredEvents = eventsList.filter(event => 
            (!filtersList.companyName || event['v_corporate_actions.long_name_en'] === filtersList.companyName) &&
            (!filtersList.symbol || event['v_corporate_actions.symbol'] === filtersList.symbol) &&
